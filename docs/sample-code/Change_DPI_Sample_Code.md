...
namespace DpiScaling
{
    ...
    {
        // DPI scaling values array (same as C++ version)
        ...
        public static DPIScalingInfo GetDPIScalingInfo(LUID adapterId, uint sourceId)
        {
            ...
            if (result == 0) // ERROR_SUCCESS
            {
                ...
                if (DpiVals.Length >= minAbs + requestPacket.maxScaleRel + 1)
                {
                    ...
                    dpiInfo.Minimum = DpiVals[0]; // Always 100
...
namespace DpiScaling
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                Console.WriteLine("Windows DPI Scaling Tool\n");

                // Get all active display paths and modes
                if (!DpiHelper.GetPathsAndModes(out List<DpiHelper.DISPLAYCONFIG_PATH_INFO> paths,
                    out List<DpiHelper.DISPLAYCONFIG_MODE_INFO> modes))
                {
                    Console.WriteLine("Failed to get display configuration.");
                    return;
                }

                Console.WriteLine($"Found {paths.Count} active display(s):\n");

                // List all displays with their current DPI settings
                for (int i = 0; i < paths.Count; i++)
                {
                    var path = paths[i];
                    var sourceInfo = path.sourceInfo;

                    // Get DPI scaling info for this display
                    var dpiInfo = DpiHelper.GetDPIScalingInfo(sourceInfo.adapterId, sourceInfo.id);

                    if (dpiInfo.IsInitialized)
                    {
                        Console.WriteLine($"Display {i + 1}:");
                        Console.WriteLine($"  Adapter ID: {sourceInfo.adapterId.HighPart:X8}{sourceInfo.adapterId.LowPart:X8}");
                        Console.WriteLine($"  Source ID: {sourceInfo.id}");
                        Console.WriteLine($"  Current DPI: {dpiInfo.Current}%");
                        Console.WriteLine($"  Recommended DPI: {dpiInfo.Recommended}%");
                        Console.WriteLine($"  Minimum DPI: {dpiInfo.Minimum}%");
                        Console.WriteLine($"  Maximum DPI: {dpiInfo.Maximum}%");

                        // Try to get the unique display name
                        var uniqueName = DpiHelper.GetDisplayUniqueName(path.targetInfo.adapterId, path.targetInfo.id);
                ...
                // Example: Change DPI for a specific display
                Console.WriteLine("\nWould you like to change the DPI for any display? (y/n)");
                if (Console.ReadLine()?.ToLower() == "y")
                {
                    Console.Write("Enter display number (1, 2, etc.): ");
                    if (int.TryParse(Console.ReadLine(), out int displayNum) &&
                        displayNum > 0 && displayNum <= paths.Count)
                    {
                        var path = paths[displayNum - 1];
                        var sourceInfo = path.sourceInfo;
                        var currentDpiInfo = DpiHelper.GetDPIScalingInfo(sourceInfo.adapterId, sourceInfo.id);

                        Console.WriteLine($"\nCurrent DPI for Display {displayNum}: {currentDpiInfo.Current}%");
                        Console.WriteLine("Available DPI values: 100, 125, 150, 175, 200, 225, 250, 300, 350, 400, 450, 500");
                        Console.Write($"Enter new DPI value ({currentDpiInfo.Minimum}-{currentDpiInfo.Maximum}): ");

                        if (uint.TryParse(Console.ReadLine(), out uint newDpi))
                        {
                            Console.WriteLine($"\nSetting DPI to {newDpi}%...");

                            if (DpiHelper.SetDPIScaling(sourceInfo.adapterId, sourceInfo.id, newDpi))
                            {
                                Console.WriteLine("DPI changed successfully!");
                                Console.WriteLine("Note: You may need to sign out and back in for changes to take full effect.");
                            }
                            else
                            {
                                Console.WriteLine("Failed to change DPI. The value might be out of range or access was denied.");
                            }
                        }
                    }
                }
